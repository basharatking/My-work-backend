# PDFPilot Backend v2.0 — main.py
# Deploy on Replit (Python 3.11)
# Rebranded from FallPDF → PDFPilot
# Bug fixes: PDF-to-Excel smart grouping (no page-wise sheets), AI endpoints
...
def open_fitz(data: bytes):
    if not _FITZ_OK:
        raise HTTPException(500, "PDF engine (PyMuPDF) not available.")
...
def ai_call(system: str, prompt: str, max_tokens: int = 1500) -> str:
    if not _AI_OK:
        raise HTTPException(503, "AI Tools unavailable. Add HF_TOKEN in Replit Secrets → Environment Variables.")
    ...
    try:
        ...
        if not resp.ok:
            ...
            raise HTTPException(502, f"AI processing failed: {msg}")
        result = resp.json()
        ...
        raise HTTPException(502, f"AI processing error: {str(e)}")
...
async def ai_summary(file: UploadFile = File(...), length: str = Form("medium")):
    ...
    if not text.strip():
        raise HTTPException(400, "No readable text found. This may be a scanned PDF.")
    lens = {
        "short": "2–3 sentences",
        "medium": "1 clear paragraph",
        "long": "2–3 paragraphs covering all key sections",
    }
    result = ai_call(
        "You are a professional document summarizer. Return only the summary — no preamble, no labels.",
...
async def ai_quiz(
    ...
    result = ai_call(
        "You are a quiz creator. Return only numbered questions with options A–D and mark the correct answer with ✓.",
        f"Create {count} {difficulty}-difficulty multiple choice questions from this document:\n\n{text[:8000]}",
    )
    return {"result": result}
...
async def ai_keypoints(file: UploadFile = File(...)):
    ...
    if not text.strip():
        raise HTTPException(400, "No readable text found.")
    result = ai_call(
        "You are an expert at identifying the most important information. Return a numbered list of key points only.",
...
async def rotate_pdf(
    ...
    for i, page in enumerate(doc):
        apply = (
            pages == "all"
            or (pages == "odd"  and i % 2 == 0)
            or (pages == "even" and i % 2 == 1)
        )
        if apply:
...
async def compress_pdf(file: UploadFile = File(...), level: str = Form("medium")):
    ...
    return stream_file(
        compressed,
        "application/pdf",
        f"compressed_{stem(file.filename)}.pdf",
        {
...
async def add_watermark(
    ...
    for page in doc:
        ...
        if position == "center":
            ...
            page.insert_text(
                (w - tw - 20, h - 20), text,
                fontsize=font_size * 0.6, color=(0.4, 0.4, 0.4),
                fill_opacity=opacity, overlay=True,
            )
        else:  # bottom-center
            tw = fitz.get_text_length(text, fontsize=font_size * 0.6)
...
# ── PDF to Excel — FIXED smart grouping (no page-wise sheets) ─────────
...
async def pdf_to_excel(file: UploadFile = File(...), mode: str = Form("smart")):
    ...
    # Styles
    hdr_fill  = PatternFill("solid", fgColor="2A5BFF")
    hdr_font  = Font(bold=True, color="FFFFFF", size=11, name="Calibri")
    hdr_align = Alignment(horizontal="center", vertical="center", wrap_text=True)
    alt_fill  = PatternFill("solid", fgColor="EEF2FF")
    thin      = Side(style="thin", color="C7D4FF")
    ...
    # KEY FIX: group ALL tables across ALL pages by their header signature
    # Same headers from any page → same sheet (rows appended)
    # This is the main bug fix over the old version which created one sheet per page
    table_groups: dict = {}   # hkey → {sheet_name, header, rows, col_count}
    text_lines:   list = []
        ...
        with pdfplumber.open(io.BytesIO(data)) as pdf:
            for pn, page in enumerate(pdf.pages, start=1):
                ...
                if mode in ("smart", "tables"):
                    # Try strict line strategy first, fall back to text strategy
                    ...
                    for tbl in tables:
                        ...
                        # CRITICAL FIX: if same header seen before → append rows (not new sheet)
                        ...
                        # Always append — never create duplicate sheets for same header
                        table_groups[hkey]["rows"].extend(data_rows)

                # Text fallback
                if mode == "text" or (mode == "smart" and not (
                    page.extract_tables({"vertical_strategy": "lines_strict", "horizontal_strategy": "lines_strict"}) or
    ...
    if table_groups:
        for idx, (hkey, grp) in enumerate(table_groups.items(), start=1):
            ...
            ws.cell(
                row=1, column=grp["col_count"] + 2,
                value=f"Total rows: {len(grp['rows'])}"
            ).font = Font(italic=True, color="8A90AA", size=10)
...
async def pdf_to_pptx(file: UploadFile = File(...)):
    ...
    for page in doc:
        ...
        slide.shapes.add_picture(
            io.BytesIO(img), PInches(0), PInches(0),
