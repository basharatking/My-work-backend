# RunDocs Backend v1.0 — main.py
# Deploy on Replit (Python 3.11)
# Fully rebranded from PDFPilot → RunDocs
# Key fix: PDF-to-Excel smart grouping (tables merged by header, NOT page-wise)
...
def ai_call(system: str, prompt: str, max_tokens: int = 1500) -> str:
    ...
    try:
        ...
        if resp.status_code == 401:
            raise HTTPException(503, "Invalid HuggingFace token. Check HF_TOKEN in Secrets.")
        if not resp.ok:
...
async def compress_pdf(file: UploadFile = File(...), level: str = Form("medium")):
    ...
    buf = io.BytesIO()
    doc.save(
        buf, garbage=4, deflate=True, deflate_images=True,
        deflate_fonts=True, clean=True, linear=True, expand=0,
    )
    compressed = buf.getvalue()
    comp_size = len(compressed)
    pct = round((1 - comp_size / orig_size) * 100, 1) if orig_size else 0
    return stream_file(
        compressed, "application/pdf", f"compressed_{stem(file.filename)}.pdf",
        {
...
async def protect_pdf(file: UploadFile = File(...), password: str = Form(...)):
    ...
    buf = io.BytesIO()
    doc.save(
        buf, encryption=fitz.PDF_ENCRYPT_AES_256,
        owner_pw=password + "_owner", user_pw=password, permissions=perm,
    )
...
# ── PDF to Excel — SMART GROUPING FIX ────────────────────────────────
# Core fix: tables grouped by header signature across ALL pages.
# Same headers from multiple pages → single sheet (rows appended).
# This eliminates the old bug where each page created a new sheet.
...
async def pdf_to_excel(file: UploadFile = File(...), mode: str = Form("smart")):
    ...
    # KEY FIX: dict keyed by header signature → rows appended across all pages
    table_groups: dict = {}
    text_lines:   list = []
            ...
            for pn, page in enumerate(pdf.pages, start=1):
                ...
                if mode in ("smart", "tables"):
                    ...
                    for tbl in tables:
                        ...
                        # SMART GROUPING: same header → append, not new sheet
    ...
    if table_groups:
        for idx, (hkey, grp) in enumerate(table_groups.items(), start=1):
            ...
            ws.cell(row=1, column=grp["col_count"] + 2, value=f"Total rows: {len(grp['rows'])}").font = Font(italic=True, color="9CA3AF", size=10)
        ...
        ws.cell(row=2, column=1, value="Tip: Make sure the PDF contains selectable text (not scanned images).")
